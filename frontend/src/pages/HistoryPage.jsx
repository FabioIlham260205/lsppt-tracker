...
function SummaryCard({ type, label, value }) {
  const icons = {
    total: (
  ...
  return (
    <div className={`rounded-lg p-4 ${SUMMARY_CARDS_BG[type]}`}>
      <div className="flex items-center gap-2">
        <span className={SUMMARY_CARDS_TEXT[type]}>{icons[type]}</span>
        <span className={`text-xs font-medium ${SUMMARY_CARDS_LABEL[type]}`}>{label}</span>
      </div>
      <p className={`mt-2 text-2xl font-bold ${SUMMARY_CARDS_NUM[type]}`}>{value}</p>
    </div>
  );
}
...
function StatusDonutChart({ data }) {
  const { completed, inProgress, onHold, testing, plan } = data;
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-2 sm:grid-cols-1">
        {segments.map((segment) => (
          <div key={segment.key} className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-slate-600">{DONUT_LABELS[segment.key]}</span>
            <span className="ml-auto text-sm font-semibold text-slate-800">{segment.value}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function HistoryPage() {
  ...
  const summary = useMemo(() => {
    ...
    const completed = filtered.filter(
      (r) => r.status === 'Completed' || r.status === 'Completed Testing',
    ).length;
    const inProgress = filtered.filter(
      (r) => r.status === 'In Progress' || r.status === 'Documentation',
    ).length;
    const onHold = filtered.filter((r) => r.status === 'On Hold').length;
    const testing = filtered.filter(
      (r) =>
        r.status === 'Testing' ||
        r.status === 'On Queue Testing' ||
        r.status === 'Completed with Feedback',
    ).length;
    const plan = filtered.filter((r) => r.status === 'Plan').length;
    return { total, completed, inProgress, onHold, testing, plan };
  }, [filtered]);
        ...
        {!loading && !error && !dateInvalid && (
          <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              <SummaryCard type="total" label="Total Tasks" value={summary.total} />
              <SummaryCard type="completed" label="Completed" value={summary.completed} />
              <SummaryCard type="inProgress" label="In Progress" value={summary.inProgress} />
              <SummaryCard type="onHold" label="On Hold" value={summary.onHold} />
            </div>

            <div className="mt-4 border-t border-slate-100 pt-4">
              <StatusDonutChart data={summary} />
            </div>
          </section>
        )}
